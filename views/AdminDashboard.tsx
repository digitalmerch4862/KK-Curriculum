...
const SubSectionCard: React.FC<SubSectionCardProps> = ({
  ...
  const fetchBibleText = async () => {
    // Aggressive sanitization of the reference string
    ...
    setIsFetching(true);
    try {
      // bible-api.com expects standard URL parameters
      const res = await fetch(`https://bible-api.com/${sanitizedQuery}`);
     
      if (!res.ok) {
        // Detailed error reporting
        if (res.status === 404) {
          throw new Error(`Reference "${bibleReference}" not found. Please check spelling (e.g. John 3:16).`);
        }
        if (res.status === 429) {
          throw new Error("Too many requests. Please wait a moment and try again.");
        }
        throw new Error(`The Bible API returned an error (${res.status}). It may be temporarily down.`);
      }
      ...
      if (data && data.text) {
        ...
        throw new Error(`The API returned no text for "${bibleReference}".`);
      }
    } catch (e: any) {
      console.error("Bible Fetch Detail:", e);
      // Catch network errors (CORS, DNS, Offline) specifically
      if (e instanceof TypeError && e.message === 'Failed to fetch') {
        alert("Network Error: Could not reach the Bible API. Please check your connection or try again later.");
      } else {
  ...
  return (
    <div className="bg-white p-5 md:p-6 rounded-[30px] relative shadow-sm border-2 border-transparent hover:border-pink-50 transition-all group flex flex-col min-h-[160px]">
      ...
      <div className="mb-2">
        <div className="w-full text-[10px] font-black uppercase tracking-widest text-gray-600 select-none">
          {sub.title}
        </div>
      </div>
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout }) => {
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    ...
    const password = window.prompt("Security clearance required. Enter Master Code:");
    ...
    if (!window.confirm("WARNING: This will permanently delete the lesson from the database. This action cannot be undone. Continue?")) return;
  ...
  const handleAutoSummarize = async () => {
    if (!formData.title) return alert("Please enter a Mission Name first.");
    setIsSummarizing(true);
    try {
      const content = serializeStructureToMarkdown();
      const result = await generateLessonSummary(formData.title, content);
      if (result) {
        ...
        <div className={`lg:col-span-3 space-y-6 ${editingId ? 'hidden lg:block' : 'block'}`}>
          ...
          <div className="space-y-4 overflow-y-auto lg:max-h-[calc(100vh-280px)] pr-2 scrollbar-hide">
            {filteredLessons.map(l => (
              ...
              >
                {/* Delete Button - Improved Trash Icon */}
                <button
                  onClick={(e) => handleDelete(e, l.id)}
                  className="absolute top-3 right-3 p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all opacity-0 group-hover:opacity-100 z-20 hover:scale-110 active:scale-95"
